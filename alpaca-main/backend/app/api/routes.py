...
from app.agents.protection_agent import protection_agent
...
# -------------------------------------------------------------------
# AI Stop Loss, Take Profit & Dynamic Trailing Protection Endpoints
# -------------------------------------------------------------------
...
def get_protection_status():
    ...
    states = protection_agent.get_all_states()
    return {
        "auto_trail_active": protection_agent.auto_trail_active,
        "positions_count": len(positions),
        "protection_states": states
    }

@router.post("/protection/predict-all")
def predict_all_protection(payload: Optional[Dict[str, Any]] = None):
    positions = portfolio_service.get_positions()
    regime = agent_orchestrator.current_regime
    rr = (payload or {}).get("risk_reward_ratio", 2.4)
    mult = (payload or {}).get("atr_multiplier", 1.8)
    predictions = []
    for pos in positions:
        pred = protection_agent.predict_protection(pos, regime, risk_reward_ratio=rr, atr_multiplier=mult)
        predictions.append(pred)
    return {"predictions": predictions}

@router.post("/protection/apply-all")
def apply_all_protection(payload: Optional[Dict[str, Any]] = None):
    positions = portfolio_service.get_positions()
    regime = agent_orchestrator.current_regime
    rr = (payload or {}).get("risk_reward_ratio", 2.4)
    mult = (payload or {}).get("atr_multiplier", 1.8)
    ...
    for pos in positions:
        pred = protection_agent.predict_protection(pos, regime, risk_reward_ratio=rr, atr_multiplier=mult)
        protection_agent.apply_prediction(pred)
        applied.append(pred)

    agent_orchestrator._add_event(
        "AI Protection Engine",
        "APPLY_PROTECTION_ALL",
        f"AI Stop Loss, Take Profit & Dynamic Trailing enabled for {len(applied)} active positions."
    )
    return {"success": True, "applied": applied}

@router.post("/protection/position/{symbol}/apply")
def apply_position_protection(symbol: str, payload: Dict[str, Any]):
    pos = next((p for p in portfolio_service.get_positions() if p["symbol"].upper() in (symbol.upper(), symbol.upper().replace("/", ""))), None)
    if not pos:
        raise HTTPException(status_code=404, detail=f"Position {symbol} not found")
    regime = agent_orchestrator.current_regime

    sl = payload.get("stop_loss_price")
    tp = payload.get("take_profit_price")
    rr = payload.get("risk_reward_ratio", 2.4)
    mult = payload.get("atr_multiplier", 1.8)

    pred = protection_agent.predict_protection(pos, regime, risk_reward_ratio=rr, atr_multiplier=mult)
    if sl:
        pred["stop_loss_price"] = float(sl)
    if tp:
        pred["take_profit_price"] = float(tp)

    protection_agent.apply_prediction(pred)
    agent_orchestrator._add_event(
        "AI Protection Engine",
        "SET_CUSTOM_PROTECTION",
        f"Set AI Stop Loss at ${pred['stop_loss_price']:.2f} and Take Profit at ${pred['take_profit_price']:.2f} for {symbol}.",
        symbol=symbol
    )
    return {"success": True, "protection": pred}

@router.post("/protection/toggle-auto-trail")
def toggle_auto_trail(payload: Optional[Dict[str, Any]] = None):
    enabled = payload.get("enabled", True) if payload else not protection_agent.auto_trail_active
    protection_agent.auto_trail_active = enabled
    status_str = "ACTIVE" if enabled else "PAUSED"
    agent_orchestrator._add_event(
        "AI Trailing Engine",
        "TOGGLE_AUTO_TRAIL",
        f"Autonomous AI dynamic profit trailing engine set to {status_str}."
    )
    return {"success": True, "auto_trail_active": enabled}

@router.post("/protection/trail-step")
def trail_step():
    positions = portfolio_service.get_positions()
    regime = agent_orchestrator.current_regime
    updates = protection_agent.monitor_and_trail_positions(
        positions,
        regime,
        event_callback=agent_orchestrator._add_event
    )
    return {"success": True, "updated_count": len(updates), "updates": updates}
...
def process_command_query(payload: Dict[str, Any]):
    ...
    # 0. AI Predict & Trail Command
    if ("predict" in q and ("stop" in q or "profit" in q or "tp" in q or "sl" in q or "trail" in q)) or ("trail all" in q) or ("trail profit" in q):
        positions = portfolio_service.get_positions()
        regime = agent_orchestrator.current_regime
        applied = []
        for pos in positions:
            pred = protection_agent.predict_protection(pos, regime)
            protection_agent.apply_prediction(pred)
            applied.append(pred)
        summary = ", ".join([f"{p['symbol']}: SL ${p['stop_loss_price']:.2f} / TP ${p['take_profit_price']:.2f}" for p in applied])
        return {
            "answer": f"🤖 AI Prediction Complete: Calculated ATR & Regime-based protection for {len(applied)} positions: {summary}. Dynamic Profit Trailing is ACTIVE.",
            "data": applied
        }
    ...
    if "take profit" in q or "tp " in q or "target" in q:
        ...
        ai_info = pos.get("ai_protection", {}) if pos else {}
        extra = f" (AI Target: {ai_info.get('target_gain_pct', 0):+.1f}% | R:R {ai_info.get('risk_reward_ratio', 2.4)}:1)" if ai_info else ""
        return {
            "answer": f"AI Risk Shield: {sym} Take Profit level is {tp_str}{extra}.",
            "data": {"symbol": sym, "take_profit": pos.get("take_profit_price") if pos else None, "ai_protection": ai_info, "source": "ALPHA_HUNTER_AI"}
        }
    ...
    if "stop loss" in q or "stoploss" in q or "trail" in q or "sl " in q:
        ...
        ai_info = pos.get("ai_protection", {}) if pos else {}
        stage = ai_info.get("trailing_stage", "INITIAL")
        locked = ai_info.get("profit_locked_amount", 0.0)
        return {
            "answer": f"AI Trailing Shield: {sym} active Stop Loss is {sl_str} (Stage: {stage}, Profit Locked: +${locked:.2f}).",
            "data": {"symbol": sym, "stop_loss": pos.get("stop_loss_price") if pos else None, "ai_protection": ai_info, "source": "ALPHA_HUNTER_AI"}
        }
