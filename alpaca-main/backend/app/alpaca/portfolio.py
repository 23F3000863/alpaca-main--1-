    ...
    def get_positions(self, symbol: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for p in positions:
                ...
                # Resolve AI Protection (Stop Loss, Take Profit, and Profit-Ratchet Trailing)
                ai_prot = protection_agent.get_protection_state(norm_sym)
                if not ai_prot:
                    temp_pos = {
                        "symbol": norm_sym,
                        "side": side,
                    ...
                    ai_prot = protection_agent.predict_protection(temp_pos)
                    protection_agent.apply_prediction(ai_prot)

                # Prioritize Alpaca broker orders if active, otherwise use AI Protection
                sl_price = prot.get("stop_loss_price") or (ai_prot.get("stop_loss_price") if ai_prot else None)
                tp_price = prot.get("take_profit_price") or (ai_prot.get("take_profit_price") if ai_prot else None)
                trailing_val = prot.get("trailing_stop") or (sl_price if ai_prot and ai_prot.get("trailing_stage") in ("PROFIT_TRAILED", "BREAK_EVEN_LOCKED", "RUNNER_MODE") else None)

                # Dynamic Trailing Status Text
                if prot.get("trailing_stop"):
                    trailing_status = f"ALPACA TRAILING (${prot.get('trailing_stop'):.2f})"
                elif ai_prot:
                    stage = ai_prot.get("trailing_stage", "INITIAL")
                    locked_amt = ai_prot.get("profit_locked_amount", 0.0)
                    if stage == "PROFIT_TRAILED":
                        trailing_status = f"⚡ TRAILED: ${sl_price:.2f} (LOCKED +${locked_amt:.2f})"
                    elif stage == "BREAK_EVEN_LOCKED":
                        trailing_status = f"🛡️ BREAK-EVEN LOCKED (${sl_price:.2f})"
                    elif stage == "RUNNER_MODE":
                        trailing_status = f"🚀 RUNNER MODE (LOCKED +${locked_amt:.2f})"
                    else:
                        trailing_status = f"🎯 AI ATR TRAIL ACTIVE (${sl_price:.2f})"
                else:
                    trailing_status = "ALPHA HUNTER ADAPTIVE TRAILING"
                ...
                pos_dict = {
                    ...
                    "stop_loss_type": prot.get("stop_loss_type") or ("AI_ADAPTIVE_STOP" if sl_price else None),
                    "take_profit_price": tp_price,
                    "take_profit_order_id": prot.get("take_profit_order_id"),
                    "take_profit_type": prot.get("take_profit_type") or ("AI_TARGET" if tp_price else None),
                    "trailing_stop": trailing_val,
                    "trailing_order_id": prot.get("trailing_order_id"),
                    "trailing_status": trailing_status,
                    "ai_protection": ai_prot,
                    "risk_score": 12.0,
                    "broker": "ALPACA",
                    "environment": "PAPER",
